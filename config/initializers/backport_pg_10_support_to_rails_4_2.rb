...
#
# Monkey-patch the rejected (due to Rails legacy support policy)
# Rails 4.2 patch at https://github.com/rails/rails/pull/31330
#
      ...
      module SchemaStatements
        ...
        def reset_pk_sequence!(table, pk = nil, sequence = nil)
          ...
          if pk && sequence
            ...
            max_pk = select_value(
              "SELECT MAX(#{quote_column_name pk}) \
               FROM #{quote_table_name table}"
            )
            if max_pk.nil?
              if postgresql_version >= 100_000
                minvalue = select_value(
                  "SELECT seqmin    \
                   FROM pg_sequence \
                   WHERE seqrelid = #{quote(quoted_sequence)}::regclass"
                )
              else
                minvalue = select_value(
